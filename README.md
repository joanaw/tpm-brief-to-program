...
executable program plans, using an AI agent as a thinking partner, not a
replacement for TPM judgment.
...
| `CLAUDE.md` | The methodology: reasoning log, judgment calls, 8-step reuse pattern |
...
1. **Characterize the document type.** Vision briefs, specs and strategy memos
   each require different responses
2. **Run three filters:** stated facts / assumptions / unresolved decisions
3. **Run AI readiness filters** (AI/ML initiatives only): problem fit, data and
   labels, evaluation, autonomy level, cost profile, failure impact
4. **Flag "I think" statements** on compliance, legal, or dependency items;
...
7. **Split metrics into leading and lagging.** Leading indicators save
   programs, lagging indicators evaluate them
8. **End with concrete immediate actions:** no more than 5, owned and
   time-bound

Step 3 catches failure modes specific to AI programs (no baseline, missing labels,
undefined autonomy, recurring costs). Skip it for briefs that don't involve AI/ML.
...
2. Read `CLAUDE.md` and understand the 8-step pattern and the judgment call table
3. Apply the three filters to your brief (stated / assumed / unresolved)
4. If the brief involves AI/ML, run the AI readiness filters
5. Use the workstream decomposition and phasing approach as a starting structure
6. Adapt the success metrics framework to your domain

AI speeds up the structuring work (workstreams, phasing, metrics) when given the
right context. Characterizing the brief and deciding what counts as verified
...
regulatory programs, or any domain where a senior leader has a vision and a team
needs to execute it.
...
Built by Joanna, a TPM specialising in AI/ML, agentic workflows, and AI Safety
operations. Exploring what it means to make AI ambiguity executable.

[AgentRed-Light](https://github.com/joanaw/agent-red-light) is a related
project: a guardrail test suite for AI agents.
