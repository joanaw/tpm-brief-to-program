...
>
> **Revised September 24, 2026 (v2):** added Section 0 (AI Readiness Check), Open Questions 14–16, running costs in Open Question 12, a note on metric targets, and related phasing changes.

---

## 0. AI Readiness Check

Runs the AI readiness filters (methodology Step 3) on the brief. Where a finding is already covered elsewhere in this analysis, the table points there.

| Filter | Finding from the brief | Status |
|--------|------------------------|--------|
| Problem fit | States a problem, but vaguely ("leaving a lot of signal on the table"); the brief leads with the solution ("leverage AI"). Churn detection is a valid ML use case, but CS teams may already track signals (usage drops, ticket spikes) that a rules-based V1 could capture. | ⚠️ Build a rules baseline first (Phase 0) |
| Data and labels | "Churn patterns" are mentioned, but not whether churn events are recorded with dates and reasons. The account-based language suggests B2B (not stated). If so, churn events may be few, limiting training data. See Assumptions (Section 3). | ⚠️ Unverified. See Open Question 14 |
| Evaluation | No success definition beyond "in market by Q3". The precision/recall targets in Section 5 were set without a baseline or base rate. | ❌ Missing. See Open Questions 15–16 |
| Autonomy level | "Proactive", "recommending next best actions" and "AI agents" span everything from alerts to autonomous action. See Open Question 4. | ❌ Undefined |
| Cost profile | Budget is TBD; recurring costs (inference, monitoring, labeling) are not mentioned. | ❌ Missing. See Open Question 12 |
| Failure impact | Not addressed. A false churn flag wastes CSM time; a missed one loses revenue; sharing scores with Sales could change how customers are treated (see Workstream 4). | ⚠️ Needs assessment |

**Effect on the plan:** V1 stays scoped to churn alerts for CS (inform/recommend only, consistent with deferring agents to post-Q3). A rules-based baseline is built in Phase 0 as the benchmark, and a small random holdout is added to the Phase 2 alpha so impact can be measured.
...
- Define model requirements for each use case (churn prediction, expansion opportunity scoring, next-best-action (NBA) recommendation)
...
12. **What is the budget, covering both build and ongoing running costs?** "TBD but this is a priority" is not a budget. Third-party ML tooling, cloud infrastructure, and potential vendor costs need a number before architecture decisions can be made. Include recurring costs (inference, monitoring, labeling); an order-of-magnitude estimate is enough to choose between build and buy.
13. **Are there dedicated PM and design resources,** or is this expected to run on engineering bandwidth alone?

**AI Readiness** *(added in v2, see Section 0)*

14. **Are churn events recorded with dates and reasons, and how many per year?** This decides whether a trained model is feasible for V1.
15. **How do CS teams spot at-risk accounts today?** That is the baseline the model has to beat.
16. **What error rate is acceptable, and which error is worse:** a missed churn or a false alarm?
...
- [ ] Build a rules-based churn baseline with the Data Team (benchmark for the V1 model)
- [ ] Answer AI readiness questions 14–16
...
- Keep a small random holdout of flagged accounts without proactive outreach, so impact can be measured against the "vs. control" lagging metric
...
> **Note (v2):** Precision/recall targets are placeholders until the churn base rate and the rules baseline are known (see Open Questions 14–15). To show value, the model must beat both.
...
| % of at-risk flags that receive a CSM action within 5 business days (excluding holdout accounts) | >70% |
